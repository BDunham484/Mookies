...
var getMovies = function (searchQuery) {
    ...
    var apiUrl = "https://imdb-api.com/en/API/AdvancedSearch/k_aaaaaaaa/?title_type=feature&genres=" + searchQuery + "&title=" + searchQuery
...
var getMovieResults = function (searchQuery, num) {
    for (var i = 0; i < 5; i++) {
        ...
        var apiUrl = "https://imdb-api.com/en/API/AdvancedSearch/k_aaaaaaaa/?title_type=feature&genres=" + searchQuery + "&title=" + searchQuery + "&count=" + randoOffsetNum
        //fetches data from open library and saves the necessary data to variable
        fetch(apiUrl).then(function (response) {
            if (response.ok) {
                response.json().then(function (data) {
                    ...
                    var starsArr = [];
                    if (data.results[index].starList.length === null) {
                        var stars = ""
                    } else {
                        for (var i = 0; i < 3; i++) {
...
var createMovieModals = function (title, starsArr, movieImg, index) {
    ...
    modalOpenLibraryLink.setAttribute("target", "_blank");
    modalOpenLibraryLink.textContent = "Where to stream"
    modalLowerContent.appendChild(modalOpenLibraryLink);
